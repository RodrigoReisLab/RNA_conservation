...
#SBATCH --error=RNA-SCoRE_job.err
#SBATCH --output=RNA-SCoRE_job.out
#SBATCH --job-name=RNA-SCoRE_job
...
do
	...
	then
		...
		perl ${curDir}/RNA-SCoRE.pl -e _motif.sto -d 0 --mt 0.5 -t 0.75 --gc 0.30 ${i}_motif.sto ${i}_motif_cleaned.sto ${i}_motif_evaluated.tsv >> ${baseDir}/allClusters_evaluation.tsv
