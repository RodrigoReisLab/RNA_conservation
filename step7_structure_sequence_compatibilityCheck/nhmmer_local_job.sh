...
# /rscape_v2.0.0.q/lib/hmmer/easel/miniapps/esl-reformat -o ${clusterName}.fa fasta ${clusterName}.sto
...
do
	...
	then
	        ...
	        /rscape_v2.0.0.q/lib/hmmer/easel/miniapps/esl-sfetch ${clusterName}.sto $rep > ${clusterName}_one.fa
	        /rscape_v2.0.0.q/lib/hmmer/easel/miniapps/esl-alimanip --seq-k list.txt ${clusterName}.sto > ${clusterName}_one.v0.sto
	        /rscape_v2.0.0.q/lib/hmmer/easel/miniapps/esl-alimask --gapthresh 0.9 -g ${clusterName}_one.v0.sto > ${clusterName}_one.sto
	        echo -e "Input files prepared for performing nhmmer for cluster: ${clusterName}\nPerforming nhmmer..!!\n"
	        /rscape_v2.0.0.q/lib/hmmer/src/hmmbuild ${clusterName}_one.hmm ${clusterName}_one.fa
	        /rscape_v2.0.0.q/lib/hmmer/src/hmmsearch -A ${clusterName}_nhmmer.sto ${clusterName}_one.hmm ${clusterName}.fa > ${clusterName}_hmmsearch.log
	       	echo -e "nhmmer step completed!\nRunning R-scape with cacofold..\n";
		if [[ ! -d  ${clusterName}_rscape_nhmmer ]]
		then
		        mkdir ${clusterName}_rscape_nhmmer
		fi
	        /rscape_v2.0.0.q/bin/R-scape --cacofold --outdir ${clusterName}_rscape_nhmmer ${clusterName}_nhmmer.sto > ${clusterName}_rscape.log
