...
#SBATCH --error=chloro_evalStruct_v2.err
#SBATCH --output=chloro_evalStruct_v2.out
#SBATCH --job-name=chloro_evalStruct_v2
...
if [ -f ${baseDir}/allClusters_evaluation_v2.tsv ]
then
	echo -e "Found allClusters_evaluation.tsv in the baseDir. Thus deleting it!\n";
	rm ${baseDir}/allClusters_evaluation_v2.tsv;
fi
...
do
	...
	then
		...
		perl ${curDir}/eval_rnaStruct_v2.pl -e _motif.sto -d 0 --mt 0.5 -t 0.75 --gc 0.30 ${i}_motif.sto ${i}_motif_cleaned_v2.sto ${i}_motif_evaluated_v2.tsv >> ${baseDir}/allClusters_evaluation_v2.tsv
